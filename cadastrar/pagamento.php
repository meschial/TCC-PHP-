        ...
        $apelido = $_SESSION["tcc"]["apelido"];
         if ( isset ( $p[2] ) ) {
          ...
          $sql = "select date_format(rota.data_inicio, '%d/%m/%Y') data_inicio, item.id, produto.nome, sum(produto.quantidade) as total, rota.cidade_inicio, rota.cidade_fim, rota.valor from produto 
          inner join item on produto.item_id = item.id
          inner join rota on item.rota_id = rota.id
    ...
    <form method="post" action="salvar/venda">
      ...
      <div class="form-row">
        <div class="form-group col-md-3">
            ...
            <input  type="text"  value="<?=$cidade_inicio;?>" readonly class="form-control">
        </div>
        <div class="form-group col-md-3">
